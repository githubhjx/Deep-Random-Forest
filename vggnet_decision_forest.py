...
def conv_op(input_op, name, kh, kw, n_out, dh, dw, p, wp):
    ...
    with tf.name_scope(name) as scope:
        ...
        kernel = kernels.copy()
        conv = tf.nn.conv2d(input_op, kernel[wp], (1, dh, dw, 1), padding='SAME')
...
with tf.device('/gpu:0'):
    for i in range(N_TREE/2):
        w_ensemble2.append(init_weights([512 * 7 * 7, 4096]))
        w_d_ensemble2.append(init_prob_weights([4096, N_LEAF], -1, 1))
        w_l_ensemble2.append(init_prob_weights([N_LEAF, N_LABEL], -2, 2))
...
with tf.device('/gpu:0'):
    decision_p_e1, leaf_p_e1 = model(X, w_ensemble, w_d_ensemble,
                                   w_l_ensemble, p_keep_conv, p_keep_hidden)
with tf.device('/gpu:1'):
    decision_p_e2, leaf_p_e2 = model(X, w_ensemble2, w_d_ensemble2,
                                   w_l_ensemble2, p_keep_conv, p_keep_hidden)

decision_p_e = decision_p_e1 + decision_p_e2
leaf_p_e = leaf_p_e1 + leaf_p_e2
